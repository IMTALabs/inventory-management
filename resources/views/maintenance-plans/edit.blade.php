    ...
    <div class="content">
        ...
        <div class="block block-rounded">
            ...
            <div class="block-content block-content-full">
                <form id="create" action="{{ route('maintenance-plans.update', ['maintenancePlan' => $maintenancePlan]) }}" method="POST">
                    @csrf
                    @method('put')
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <input type="text"
                                   ...
                                   value="{{ old('maintenance_plan_name') ?? $maintenancePlan->plan_name }}">
                            @error('plan_name')
                            ...
                            <select
                                ...
                                @foreach($equipmentsCompact as $equipment)
                                    <option value="{{ $equipment->id }}"
                                            @if($equipment->id == old('equipment_id') || $equipment->id == $maintenancePlan->equipment_id) selected @endif>
                            ...
                            <select class="form-select form-control-alt @error('frequency') is-invalid @enderror"
                                ...
                                @foreach(\App\Enums\MaintenancePlanFrequencyEnum::cases() as $frequency)
                                    <option value="{{ $frequency->value }}"
                                            @if($frequency->value == old('frequency') || $frequency->value == $maintenancePlan->frequency->value) selected @endif>
                        ...
                        <div class="col-12">
                            ...
                            <textarea rows="10"
                                      ...
                                      placeholder="..."
                                      name="description">{{ old('description') ?? $maintenancePlan->description }}</textarea>
                            @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
